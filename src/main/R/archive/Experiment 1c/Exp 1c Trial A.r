...
colnames(fieldresults.df)
sum(fieldresults.df$eggs_24.02.2006[fieldresults.df$Field=='E'])
...
all.results.list<-outputTrialBResults(layout.df, inp.df, inp.df$iteration, numReplicants, baseFilename, experimentNumber, trialId) 

#Chisquared comparison between sim and field results....
chisq.results<-calculateChiSquaredGoodnessOfFits(layout.df, fieldresults.df, summary.df, summary.df$iteration, numReplicants, experimentDir, experimentNumber)
chisq.df<-data.frame.ChisqResults(chisq.results)
writeDataFrame(list(chisq.df),  basefilename, experimentNumber, trialId, title="chisq-results")
...
par(mfrow=c(1,2))
plotEggsPerPlantBoxes(layout.df$eggperplant.1m, layout.df$X1m_dens, layout.df, "1x1m Density")
plotEggsPerPlantBoxes(layout.df$eggperplant.6m, layout.df$X6m_dens, layout.df, "6x6m Density")

plotEggsPerPlantBoxes<-function(eggsPerPlantFactor, densityFactor, data.df, main) {
	boxplot(eggsPerPlantFactor~densityFactor, data=data.df, ylim=c(0,20),xaxt="n", main=main, xlab="Number of plants", ylab="Eggs per plant")
	#axis(side=1, at=c(1, 4, 16))
	axis(side=1, at=c(1, 2, 3, 4), lab=c(1, 4, 16, 40))
}
