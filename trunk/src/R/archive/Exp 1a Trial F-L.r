...
#Show the differences between the CENTRE_RATIO s
...
#Show differences in border crossings
plotBorderCrossingResults(crossings.df, baseFilename, experimentNumber, trialId)


#Now look at the frequencies in a bit more detail
results.list<-plotFrequencyOfFactor(crossings.df, "L", "B", "d", 5, newPlot=FALSE)
...
#Old stuff.....
#Just plot the frequency of 
quartz(width=10, height=10)
N<-sum(crossings.hist$counts)
plot(crossings.hist, main=sprintf("N=%d", N))
summary(crossings.hist)


dataframes<- createSummaryDataFrame(baseFilename, experimentNumber, trialId)
