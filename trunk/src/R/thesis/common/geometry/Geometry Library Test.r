...
plotAreaCovered(layout.df, 1:8, 100)


coord.displacementDistance(c(0, 10), c(10, 10))

radius<-50
layout.df<-Resourcelayout.createLayout(100, 4, 0, 0, edges=F)
cx<-layout.df$x
cy<-layout.df$y


estimates<-c()
x<-seq(from=100, to=1500, by=100)
for (N in x) {
	est<-Geomtery.estimateAreaOfCircles(layout.df$x, layout.df$y, radius, n=100)
	estimates<-c(estimates, est)
}

plot(estimates~x, type="l", col="blue", xlim=c(0, max(x)))
...
Geomtery.estimateAreaOfCircles(layout.df$x, layout.df$y, itr@resourceRadius, n=100)
...
for (i in 1:8) {
	ids <-1:i
	plotAreaCovered(layout.df, ids)
}

plotAreaCovered<-function(layout.df,ids,  patchSize) { 
	cx<-layout.df$x
	cy<-layout.df$y
	minx<-min(cx)-radius
	miny<-min(cy)-radius
	...
	par(mar=c(1, 1, 1, 1))
	plot(layout.df$y[ids]~layout.df$x[ids], pch=NA, xlim=c(0, 200), ylim=c(0, 200), axes=F)
	rect(minx, miny, maxx, maxy, border="blue")
	
	box()
	symbols(layout.df$x[ids],layout.df$y[ids], rep(50, length(layout.df$x[ids])), inches=F, add=T)
	text(20, 200, sprintf("Circle Area  = %0.0f", (pi*50^2*length(ids))), cex=1.1)
	areaBounds<-(maxx-minx)*(maxy-miny)
	text(100, 200, sprintf("Bounded Area = %0.0f", areaBounds), cex=1.1)
	text(170, 200, sprintf("Patch Area = %0.0f", patchSize^2), cex=1.1)

	points(layout.df$y[ids]~layout.df$x[ids], col="blue")
	side<-patchSize

	rect(100-(side/2), 100-(side/2), 100+(side/2), 100+(side/2), border="red")
}

result<-Geometry.calculateIntersection(slope1=3, intercept1=1, slope2=-3, intercept2=30)
