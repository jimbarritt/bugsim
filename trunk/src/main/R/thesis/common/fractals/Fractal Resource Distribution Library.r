...
frez.plotUpperVsLowerTailed<-function(D, r0) {
	...
	mtext(expression("Probability (P)"), 2, 4, cex=2)
...
frez.generatePointPattern<-function(type="OGATA", steps, randAzimuth, randStep, D, scale, minStep, maxStep, stepBy) {

	if (type=="LEVY") {
		path<-frez.levyWalk(n=steps,  randAzimuth=randAzimuth, randStep=randStep,startX=0, startY=0, minStep=minStep, maxStep=maxStep , stepBy=stepBy,fractalDimension=D)
	} else if (type=="BROWNIAN") {
		path<-frez.brownianWalk(n=steps,  randAzimuth=randAzimuth, randStep=randStep,startX=0, startY=0, minStep=1, maxStep=200 , stepBy=1, sd=1)		
	} else if (type=="OGATA") {
...
frez.levyWalk<-function(n, randAzimuth, randStep, startX, startY, minStep, maxStep, stepBy, fractalDimension) {
	...
	y<-frez.reynolds.pstepLengthList(x, minStep, fractalDimension)	
...
frez.ogata.levyWalk<-function(n, randAzimuth, randStep, startX, startY, minStep, maxStep, fractalDimension) {
	path<-list()
	currentLocation<-c(startX, startY)
	path<-appendToList(path, currentLocation)
	for (i in 1:n) {
...
frez.reynolds.pstepLengthList<-function(l, lstar, mu) {
	...
	for (i in 1:length(l)) {
		li<-l[i]
		pi<-frez.reynolds.pstepLength(li, lstar, mu)
		p<-c(p, pi)
	}
	return (p)
}
...
frez.reynolds.pstepLength<-function(l, lstar,mu) {
	p<-(mu-1)*frez.reynolds.theta(l-lstar)*lstar^(mu-1)*l^(-mu)
	return (p)
}
